...
class AddressController extends Controller
{
    ...
    public function getgovernment(Request $request){
        $data = City::where('deleted' , '0')->select('id', 'title_'.$request->lang.' as title')->get();
        $response = APIHelpers::createApiResponse(false , 200 , '' , '' , $data , $request->lang);
        return response()->json($response , 200);
    }

    public function getareas(Request $request,$id){
        $data = Area::where('city_id',$id)->where('deleted' , '0')->select('id', 'title_'.$request->lang.' as title')->get();
        $response = APIHelpers::createApiResponse(false , 200 , '' , '' , $data , $request->lang);
        return response()->json($response , 200);
    }
